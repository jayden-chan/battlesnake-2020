...
use super::{Profile};
use std::{cmp, clone::Clone, f64};
use super::super::game::{Dir, Snake, State, Point}; 
...
//This generates recursive function simulates our snake and the enemy snake taking turns, with the
...
fn minimax(self_id: &str, enemy_id: &str, depth: u8, st: &State, maximizing_player: bool, alpha: i16, beta: i16) -> (i16, Point) {
    if depth > 10 {
        let snake = st.board.snakes.get(self_id).unwrap();
        let near_food = snake.nearest_food(&st);
        let score: i16 = match near_food {
    ...
    for (pos_move, _) in temp_snake.body[0].successors(&temp_snake, &temp_st) {
        ...
        if maximizing_player {
            ...
            let (val, _) = minimax(self_id, enemy_id, depth + 1, &new_st, false, alpha, beta);
            if val > best_score {
                best_move = pos_move;
            }
            ...
            let (_, _) = snake.update_from_move(dir, &st.board.food);
            let (val, _) = minimax(self_id,enemy_id, depth + 1, &new_st, true, alpha, beta);
            if val < best_score {
                best_move = pos_move;
            }
...
impl Profile for AlphaBeta {
    fn get_move(&mut self, s: &Snake, st: &State) -> Dir {
        ...
        let (score, dir) = minimax(self_id, &enemy_id, 1, &mut st.clone(), true, MIN, MAX);
